...
uint32_t GetMagicXor(const vector<uint32_t>& array) {
  ...
  for (uint32_t element : array) {
    int number_of_ones = NumberOfOnes(element);
    if (number_of_ones <= 4) {
      sum = sum ^ element;
    }
  }
...
bool IsPointInArea(double x, double y) {
  ...
  * y - 2.5) && (x < M_PI + kEps)) {
    return true;
  } else if ((x + kEps > -M_PI) && (y <= -2 * x - 7 + kEps) && (y <= tan(x /
  2) + kEps)) {
    return true;
  } else if ((x + kEps > M_PI) && (x * x + y * y <= 25 + kEps) && (y <= tan(x
  / 2) +
  kEps)) {
    return true;
  } else {
...
int main() {
  ...
  }
  {
    ...
    // vector<string> copy_of_values = values;
